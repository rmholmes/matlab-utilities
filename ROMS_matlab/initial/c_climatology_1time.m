function [status]=c_climatology_1time(S)

%
% This function has been edited to use only one time vector and
% make it an unlimited time dimension due to problems with endDef.
%
% Ryan 13-8-13
%
...
% svn $Id: c_climatology.m 628 2012-08-07 14:28:01Z arango $
%=========================================================================%
...
if (isfield(S,'def_zeta')),
  if (S.def_zeta && ~isfield(S,'clm_time')),
    error(['C_CLIMATOLOGY - Cannot find dimension parameter: ',         ...
           'clm_time, in structure array S']);
  end
else  
  S.def_zeta=0;
  S.clm_time=0;
...
if (S.def_zeta),
 Dname.clm_time = 'clm_time';    Dsize.clm_time = nc_constant('nc_unlimited');%S.clm_time;
end
...
[ncid,status]=mexnc('create',ncname,'NC_64BIT_OFFSET');
if (status ~= 0),
  disp('  ');
...
if (S.def_zeta),
  [did.clm_time,status]=mexnc('def_dim',ncid,Dname.clm_time,Dsize.clm_time);
  if (status ~= 0),
    disp('  ');
    disp(mexnc('strerror',status));
...
if (S.def_zeta),
  Var.name          = Vname.clm_time;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time];
...
if (S.def_zeta),
  Var.name          = Vname.zeta;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.yr did.xr];
  Var.long_name     = 'sea surface height climatology';
  Var.units         = 'meter';
  Var.time          = Vname.clm_time;
  if (spherical),
...
if (S.def_v2d),
  Var.name          = Vname.ubar;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.yu did.xu];
  ...
  Var.name          = Vname.vbar;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.yv did.xv];
...
if (S.def_v3d),
  Var.name          = Vname.u;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.Nr did.yu did.xu];
  Var.long_name     = 'u-momentum component climatology';
  Var.units         = 'meter second-1';
  Var.time          = Vname.clm_time;
  if (spherical),
    Var.coordinates = strcat([Vname.ulon,' ',Vname.ulat,' ',            ...
                              Vname.s_rho,' ',Vname.clm_time]); 
  else
    Var.coordinates = strcat([Vname.ux,' ',Vname.uy,' ',                ...
                              Vname.s_rho,' ',Vname.clm_time]); 
  end
  ...
  Var.name          = Vname.v;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.Nr did.yv did.xv];
...
if (S.def_temp),
  Var.name          = Vname.temp;
  Var.type          = nc_constant('nc_double');
  Var.dimid         = [did.clm_time did.Nr did.yr did.xr];
