function [var] = GetVarSta(hname,VarOp,station,slice);
%---------------------------------------------------------------------
%---------------------------------------------------------------------
%
% This function returns a specified slice (slice) of the specified
% variable (VarOp) taken from a station file.
%
% INPUTS:
%
...
%
% slice = cell defining the slice to take, always of the form:
%
% {[z1 z2],[t1 t2]}; depth indicies and time indices, if 0 then
% full range assumed.
%
%---------------------------------------------------------------------
%
% Dependencies; none. 
%
% Ryans ROMS Matlab and netcdf Utilities 1/5/14
%
%---------------------------------------------------------------------
%---------------------------------------------------------------------
...
% $$$             z_w(:,:,:,tind) = ztmp(xi:(xi+xL-1),yi:(yi+yL-1),zi:(zi+zL));
    ...
    %isopycnal plot:
    if (vtype == 4)
% $$$         eval(['rho = netcdf.getVar(ncid,netcdf.inqVarID(ncid,''rho''),' ...
% $$$               Dom(ncid,'rho') ',''double'');']);
        eval(['rho = (sw_dens0((' ...
